...
def denormalize_partials(partials: np.ndarray, sigma_x: np.ndarray, sigma_y: np.ndarray) -> np.ndarray:
...
class Dataset:
    ...
    def std_x(self) -> np.ndarray:
        """Return standard deviation of input data as array of shape (n_x, 1)."""
        return std(self.X)
