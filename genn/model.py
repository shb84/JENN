...
def main():
    ...
    # Sample data
    X_train, Y_train, J_train = load_csv(file='../demo/demo_train_data.csv',
                                         inputs=["X[0]", "X[1]"], outputs=["Y[0]"], partials=[["J[0][0]", "J[0][1]"]])
    X_test, Y_test, J_test = load_csv(file='../demo/demo_test_data.csv',
                                      inputs=["X[0]", "X[1]"], outputs=["Y[0]"], partials=[["J[0][0]", "J[0][1]"]])
    ...
    # Training
    model = GENN.initialize(n_x=X_train.shape[0], n_y=Y_train.shape[0], deep=deep, wide=wide)
